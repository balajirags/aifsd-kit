...
**Skills get relocated, not mirrored, and no role file mentions where.** `docs/skills/<slug>/SKILL.md` in *this* repo is a real [Agent Skills](https://agentskills.io/specification) file and the authoring location. `AGENTS.md`, `docs/process.md`, `docs/team/*.agent.md`, `docs/discovery/architect.agent.md`, and `docs/templates/skill.template.md` are copied byte-identical into every kit regardless of harness — none of them name a skill path or instruct an agent to list/judge one. That's deliberate: all five supported harnesses now have a native skill mechanism that discovers and judges relevance from their own folder automatically (Copilot added this in December 2025 — see [GitHub's changelog](https://github.blog/changelog/2025-12-18-github-copilot-now-supports-agent-skills/) — it's no longer an exception), so `build-kit.sh` only needs to put the files there — no text to keep in sync, nothing to rewrite.
...
| `copilot-kit/` | `AGENTS.md` + `.github/copilot-instructions.md` | `.github/skills/<slug>/SKILL.md` |

Copilot's native discovery also recognizes `.claude/skills` and `.agents/skills` at the project level (per GitHub's own docs) — `.github/skills` was chosen here only to match the `.github/`-namespace convention this kit already uses for Copilot's `copilot-instructions.md` and `agents/`, not because the other two paths don't work.
...
| `copilot-kit/` | `.github/agents/<name>.agent.md` — GitHub's real custom-agent mechanism, a single file format spanning VS Code's chat agent picker, the GitHub.com cloud coding agent, and the Copilot CLI | None claimed — GitHub's docs don't document an isolation/fork concept for custom agents the way Claude Code/OpenCode do; `to-prd`/`to-brd`/`architect` set `target: vscode` instead, since the cloud coding agent is an autonomous, PR-opening surface unsuited to a sustained co-authoring conversation | A commented `# model:` line in the agent file |

`reviewer` additionally gets real, enforced tool restriction wherever the harness supports it — `disallowed-tools: Edit, Write, NotebookEdit` (Claude Code) / `permission: {edit: deny, bash: deny}` (OpenCode). Copilot's `reviewer.agent.md` gets a commented `# tools:` example instead of an active restriction, since this kit doesn't have a verified, stable enumeration of Copilot's built-in tool IDs to hardcode across its three surfaces (VS Code/cloud/CLI) — filling that in is left to the team.

**Note on Copilot's two independent native mechanisms — don't conflate them.** Skill discovery (`.github/skills/*/SKILL.md`, see the skills section above) and custom agents (`.github/agents/*.agent.md`, this section) are separate features that both happen to be native to Copilot now — one doesn't imply the other, they were added and are documented independently. A custom agent's frontmatter has no field that references skill files (verified against GitHub's custom-agents-configuration reference) — an agent relies on the same ambient skill auto-discovery as everything else, not an explicit per-agent declaration.
...
- **Copilot:** `.github/agents/*.agent.md` files with `target: github-copilot` (or unset) are reachable by GitHub's cloud coding agent, an autonomous surface that opens PRs on its own — don't assume every generated agent file here is VS-Code-chat-only.
