...
**Note on Copilot and skills vs. personas:** these are two independent gaps, don't conflate them. Copilot still has no native mechanism for discovering `SKILL.md`-format skill files (see the skills section above — `.github/copilot-instructions.md` covers that gap by explicit instruction). Custom agents (`.github/agents/*.agent.md`) are a separate, genuinely native mechanism for personas specifically, unrelated to skill discovery — Copilot having one doesn't mean it has the other.
