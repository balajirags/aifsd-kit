...
`docs/skills/` (empty in this kit by design) is the analogous per-project mechanism for engineering-standard rules (security, db conventions, logging, etc.) — `docs/templates/skill.template.md` defines the shape. Each skill declares a `Scope` (`Developer` \| `Reviewer` \| `Architect`, any combination) and an "Applies when" condition; developer/reviewer/architect agents each load only the skills scoped to include them, and only the ones whose condition matches what they found, not all of `docs/skills/` at once.
